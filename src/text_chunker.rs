   ...
   fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
       match &self.value {
           ...
           DataValue::Text(text) => {
               ...
               write!(f, "{}[{}]: {}", self.data_type as u8, text.len(), truncated.replace("\n", " "))
           }
...
pub enum DataValue {
   ...
   // Add additional value types as necessary
}
...
impl fmt::Display for InferenceMetadata {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "num_input_tokens={}, num_output_tokens={}", 
...
impl DataBundle {
    ...
    fn repr(&self) -> String {
        ...
        if self.items.len() == 1 {
            ...
            return format!("Bundle[{} items starting with {}]", self.items.len(), self.items[0]);
        }
    }
...
pub fn create_nonoverlapping_chunks(text: &str, chunk_length_characters: usize) -> Vec<String> {
   ...
   println!("Created {} non-overlapping chunks from {} characters, longest chunk={}", chunks.len(), text.len(), max_chunk);
   chunks
}

fn read_text_file(path_to_text_file: &str) -> std::io::Result<String> {
   fs::read_to_string(path_to_text_file)
}
...
fn normalize_special_characters(text: &str) -> String {
   if text.is_empty() {
       return String::from(text); // Return an empty String
   }
   let text = fix_line_breaks(text); // Fix line breaks
   fix_apostrophes(&text) // Fix apostrophes and return the result
}

fn get_text_chunks(document_uri: &str, document_text: &str, chunk_length_characters: usize, overlap_factor: &mut usize, normalize_special_characters_flag: bool) -> Vec<DataBundle> {
   let file_name = Path::new(document_uri).file_name().unwrap().to_string_lossy().to_string();
   ...
   if chunk_length_characters <= 0 {
      ...
      return vec![DataBundle::new(vec![data_item], ErrorCode::Undefined, Some(source))];
   }
   ...
   if *overlap_factor == 0 {
      ...
      let adjusted_chunk_length = (chunk_length_characters as f64 / *overlap_factor as f64).round() as usize;
...
fn main() {
   ...
   let text = read_text_file(document_uri).expect("Failed to read file");
   let chunks = get_text_chunks(document_uri, &text, 500, &mut 0, true);
   // Print the results
